...
class FabricDataAgentClient:
    ...
    def __init__(self, tenant_id: str, data_agent_url: str, external_credential=None):
        """
        ...
        Args:
            ...
            external_credential: Optional pre-built ``azure.identity`` credential.
                When provided, the client reuses it instead of creating its own
                ``InteractiveBrowserCredential`` — useful when the same Entra
                sign-in must also serve a different Azure resource (e.g. Azure
                OpenAI) without forcing the user to sign in twice.
        """
        ...
        if external_credential is not None:
            # Reuse an already-built credential (shared sign-in path).
            self.credential = external_credential
            self._refresh_token()
            print("✅ Authentication successful (shared credential)!")
        else:
            self._authenticate()
    ...
    def _get_openai_client(self) -> OpenAI:
        ...
        return OpenAI(
            api_key="not-used",  # Not used - we use Bearer token via default_headers
